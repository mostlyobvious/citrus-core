...
describe Citrus::Core::ExecuteBuildService do
  ...
  let(:test_runner)           { fake(:test_runner, start: fake(:exit_code)) }
  ...
  let(:success_exit_code)     { Citrus::Core::ExitCode.new(0) }
  let(:failure_exit_code)     { Citrus::Core::ExitCode.new(1) }
